...
weatherapi.open('GET', 'https://api.openweathermap.org/data/2.5/weather?id=5607916&units=imperial&APPID=09cad4cd3e9b2194a02b2c79698e72d2', true)
weatherapi.send();
weatherapi.onload = function() {
	...
	var speed = getWeather.wind.speed;
	...
	if (!getWeather.hasOwnProperty("rain.1h")) {
		...
		if (!tempObj.hasOwnProperty("1h")) document.getElementById('curr-rain').innerHTML = 'no data available';
		else document.getElementById('curr-rain').innerHTML = jsonRespObj.rain["1h"];
	}
}